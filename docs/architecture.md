...
| yfinance batch `download` | `fundamentals._batch_close_prices` | wrap-degrade — every Sortino field `None` for all | network / shape error → returns `None`; `sortino_ratio` + the informational `sortino_3y/5y/10y/20y/30y` windows (and `sortino_custom` when requested) all stay unset per ticker |
