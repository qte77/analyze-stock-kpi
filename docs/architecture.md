...
| SEC XBRL cross-validation | `sec.xbrl.enrich_snapshot_xbrl` | wrap-degrade — drop `sec_*_delta_pct` fields | called per-ticker by `enrich_snapshot_xbrl`; one ticker's XBRL fetch failure (or non-SEC-registered symbol) leaves enrichment as `{}`, snapshot survives |
...
| yfinance batch `download` | `fundamentals._batch_close_prices` | wrap-degrade — every Sortino field `None` for all | network / shape error → returns `None`; `sortino_ratio` + the informational `sortino_3y/5y/10y/20y/30y` windows (and `sortino_custom` when requested) all stay unset per ticker. A ticker the batch returns empty or all-NaN is re-downloaded once in a second batch (`_download_closes`); any still missing is logged and keeps `None` |
...
│       └── xbrl.py                   fetch_xbrl_concept / enrich_snapshot_xbrl — EDGAR XBRL companyconcept cross-validation of yfinance revenue/net-income/EPS
├── orchestrators/
...
│   ├── longshort_backtest.py         pit_fundamentals/score_at/rank_dates/rebalance_dates/select/simulate/metrics/null_percentile/fidelity + fetch_frames/main — point-in-time backfilled best/worst 25 + backtested equal-weight long/short 25/25 across five cadences, two never-spliced series: B (reconstructed, D1-D14) + A (genuine decisions via `aggregated_scores_best_and_worst.build_universe`, D15-D19), both append-only/frozen (ADR-0013)
...
A second cron (`.github/workflows/demo-snapshot.yaml`, Sunday 06:15 UTC) runs `make run UNIVERSE=qte77-watchlist`, renames the timestamped output to `results/demo/qte77-watchlist/YYYY-MM-DD.json`, rebuilds the manifest via `scripts/build_demo_manifest.py`, and commits both files to the `data` branch through the same verified-commit mechanism. It fans out this way across every **non-derived** universe in `ui/public/universes.json` (`"derived"` field absent or `false`) — the seven base universes plus `federal-contractors`.

`.github/workflows/universe-builder.yaml` runs on `demo-snapshot`'s completion (and stays gated on its `success` conclusion). Its `aggregated-scores-{best,worst}` and `enhanced-kpi-screener-{longs,shorts}` legs (`"derived": true` in `ui/public/universes.json`) pull the seven base universes' just-written `results/demo/<u>/` snapshots, run `aggregated_scores_best_and_worst.build_universe` / `enhanced_kpi_screener_longshort.build_universe` to rank/classify them, then write BOTH the preset `.txt` (next week's membership) AND `results/demo/<preset>/<date>.json` + `index.json` directly from the exact `FundamentalsSnapshot` objects that were ranked/classified (`ranked_snapshots`, `scripts/_demo_snapshot_loader._write_demo_snapshot`) — never a second, independent yfinance fetch. `demo-snapshot.yaml` explicitly excludes these four ids from its own fan-out and `workflow_dispatch` choices, so a ticker always shows the identical qte77 Score in the aggregated/derived list and its source universe's own snapshot for the same date (previously: `demo-snapshot.yaml` re-fetched these four universes itself via `make run`, recomputing a fresh, independently-timed `screener_score` that could disagree with the ranking/classification audit — e.g. a worst-list ticker outscoring a best-list one).
...
A fourth cron (`.github/workflows/portfolio.yaml`, Saturdays 12:00 UTC — no other data-branch writer runs then) runs `python -m analyze_stock_kpi.orchestrators.longshort_backtest`, which recomputes **two never-spliced series** (ADR-0013 amendment 2026-09-24, D15/D17): **series B** re-ranks every D1 base universe by the point-in-time-only `score_at` at each weekly rank date, and **series A** re-ranks the genuine `data`-branch snapshot dates with the full live qte77 Score via `aggregated_scores_best_and_worst.build_universe`. Both are **append-only / frozen** — a run only appends new dates; already-written rows/lists are never recomputed, and the summary/metrics are read back from what's actually on disk. A `method_version` bump is the only explicit full-rebuild path. Series B writes `results/series/backtest/<cadence>/YYYY.json`, `results/backtest/lists/YYYY.json`, `results/backtest/trades/<cadence>/YYYY.json` and `results/backtest/summary.json`; series A writes the parallel `results/series/backtest_genuine/<cadence>/YYYY.json`, `results/backtest_genuine/lists/YYYY.json`, `results/backtest_genuine/trades/<cadence>/YYYY.json` and `results/backtest_genuine/summary.json` — all on the `data` branch through the same verified-commit mechanism. Raw prices and statement line items never leave the run — `fetch_frames` and `write_closes_cache` cache them only under the gitignored `results/prices/` (never checked out from `data`, never matched by the commit filter). `results/backtest/lists/YYYY.json` (series B, incl. its most recent entry) stays a **purely historical, `score_bt`-ranked backfill** — the dashboard's backtest section does not display it as "today's" candidates (ADR-0014): `ui/app.js`'s `loadCurrentAggregatedCandidates` sources that panel from the `aggregated-scores-best`/`-worst` demo snapshots instead. Series A (D16) is a separate, later addition ranked with the full live `screener_score` directly (not `score_at`), so it is unaffected by ADR-0014's score-drift defect by construction — it reuses `build_universe` itself rather than a second display copy of its output.
...
| `BacktestDailyRow` / `BacktestListEntry` / `BacktestSummary` | `orchestrators/longshort_backtest.py` | Frozen data contract shared by series B (reconstructed backfill) and series A (genuine decisions) ([ADR-0013](decisions/0013-point-in-time-backtest.md), amendment 2026-09-24) — daily-marked gross/net long/short returns per cadence, the best/worst-25 per rank date, and the summary metrics/null-benchmark/fidelity block (`null`/`fidelity` may be `None` for series A); raw prices and statement line items are never persisted |

## External boundaries

- **`yfinance`** — fundamentals (`Ticker.info`) + price history (`Ticker.history`); rate-limit risk; live tests tagged `@pytest.mark.network` (excluded from default `make test`, opt in via `pytest -m network`). `_normalize_yfinance_info` in `src/analyze_stock_kpi/data_sources/fundamentals.py` divides the current percentage-shaped `info["dividendYield"]` by 100 at the fetch boundary so the rest of the codebase sees fractional yields. `fetch_universe_fundamentals` adds a batched `yf.download(tickers, period="max")` once per run, sliced per-ticker into Sortino at 1y (the composite input; unchanged), 3y, 5y, 10y, 20y and 30y windows, plus an optional operator-chosen custom `[--sortino-from, --sortino-to]` frame — see [ADR-0004](decisions/0004-price-history-composite-input.md). `_fetch_rd_to_revenue` reads `Ticker.income_stmt` per EQUITY ticker for the R&D / Revenue ratio. `_fetch_fcf_margin` reads `Ticker.cashflow` + `Ticker.income_stmt` per EQUITY ticker for the Free-Cash-Flow margin (criterion 12 of #192).
...
`sec.xbrl.fetch_xbrl_concept` additionally calls `data.sec.gov/api/xbrl/companyconcept/CIK<10>/us-gaap/<concept>.json` (same headers, same stdlib `urllib.request` client, no disk cache) to cross-validate yfinance revenue / net-income / EPS against SEC XBRL facts.
