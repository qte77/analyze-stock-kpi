...
- [x] **Backfill F&G long-term + 5s10s history deeper** — CNN F&G + the 5s10s slope now reach **2011** on the `data` branch (new `scripts/backfill_yield_curve.py` forces a full `period=max` yfinance fetch; F&G via the whit3rabbit mirror), and the demo loader (`loadYearsFromBranch`) now fetches the full year range so the charts paint it. Issue [#287](https://github.com/qte77/analyze-stock-kpi/issues/287), PRs #297 + #300; plan [`backfill-history`](plans/backfill-history.md).
- [ ] **Merge F&G long-term + 5s10s into one chart + SP500** (log secondary axis) — collapse the two long-term-context tabs, overlay SP500 price; 5s10s normalized or a third axis. Issue [#288](https://github.com/qte77/analyze-stock-kpi/issues/288), plan [`merge-longterm-charts`](plans/merge-longterm-charts.md); depends on #287.
...
- [x] **Restructure data-branch `results/` layout (group by kind)** — regrouped into `series/` + `demo/` + `audit/<universe>/`; coordinated data move (copy → redeploy → delete) kept the live demo 404-free. PR #305 (2026-06-18). Plan [`restructure-results`](plans/restructure-results.md); `demo/`→`snapshots/` rename deferred to #289.
...
- [ ] Deferred EDGAR product use cases — XBRL cross-validation [#101](https://github.com/qte77/analyze-stock-kpi/issues/101) ([plan](plans/xbrl-crossval.md)), Form 4 insider momentum [#102](https://github.com/qte77/analyze-stock-kpi/issues/102), 8-K material events [#103](https://github.com/qte77/analyze-stock-kpi/issues/103)
