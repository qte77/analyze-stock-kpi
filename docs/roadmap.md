...
- [x] **Universe coverage audit** — `scripts/audit_universes.py` + `src/analyze_stock_kpi/orchestrators/universe_audit.py` classify every bundled-preset ticker against current Yahoo data (OK / SPARSE / FAIL) — issue [#168](https://github.com/qte77/analyze-stock-kpi/issues/168), PR #174
- [x] **F&G historical backfill** — `scripts/backfill_fear_greed_whitrabbit.py` + `src/analyze_stock_kpi/data_sources/sentiment_backfill.py` extend the dashboard's long-term context from ~13 months to ~14 years via the [`whit3rabbit/fear-greed-data`](https://github.com/whit3rabbit/fear-greed-data) mirror; ADR-0005 amendment classifies as Tier-0 — issue [#164](https://github.com/qte77/analyze-stock-kpi/issues/164), PR #178
...
- [x] **5s10s yield curve slope panel** — `src/analyze_stock_kpi/data_sources/yield_curve.py` + `.github/workflows/yield-curve.yaml` (daily cron, 22:30 UTC, Tier-0 yfinance `^TNX` / `^FVX`) + new "5s10s slope" tab in the long-term-context panel. Issue [#165](https://github.com/qte77/analyze-stock-kpi/issues/165).
...
- [x] **Enhanced KPI long/short screener — Phase 2a** — `src/analyze_stock_kpi/orchestrators/enhanced_kpi_screener_longshort.py` ships 13 of the issue's 16 conjunctive-gate criteria (every gate that reads an existing `FundamentalsSnapshot` field plus criterion 14 via the new `analyst_recommendation` model field — no new HTTP). Paired output (`enhanced-kpi-screener-longs.txt` + `…-shorts.txt`); long ∩ short empty by construction. Wired into `universe-builder.yaml`, `demo-snapshot.yaml`, and `docs/demo/universes.json`. Deferred to Phase 2b: criterion 12 (FCF margin, needs `Ticker.cashflow`) and criterion 15 (tech rating, blocked on #21). PR #228, partial closure of issue [#192](https://github.com/qte77/analyze-stock-kpi/issues/192).
...
- [x] **SEC XBRL cross-validation** — `src/analyze_stock_kpi/data_sources/sec/xbrl.py` fetches SEC XBRL `companyconcept` revenue / net-income / EPS and cross-validates against yfinance, attaching `sec_revenue_delta_pct` / `sec_net_income_delta_pct` / `sec_eps_delta_pct` to every `FundamentalsSnapshot`; `total_revenue` / `net_income_to_common` promoted to permanent snapshot fields. Issue [#101](https://github.com/qte77/analyze-stock-kpi/issues/101), PR [#373](https://github.com/qte77/analyze-stock-kpi/pull/373). Plan [`xbrl-crossval`](plans/004-xbrl-crossval.md).
- [x] **Multi-window Sortino** — per-ticker `sortino_ratio` (1y, unchanged composite input) plus informational `sortino_3y/5y/10y/20y/30y` windows (`None` when price history doesn't reach back far enough), shown as detail-panel rows, plus a CLI-only custom frame via `--sortino-from`/`--sortino-to` → `sortino_custom`. PR [#394](https://github.com/qte77/analyze-stock-kpi/pull/394). Plan [`007`](plans/007-longshort-portfolio-and-multiwindow-sortino.md) (PR A).
- [x] **Hypothetical long/short model portfolio** *(superseded by the point-in-time backtest, plan 008 — see below)* — forward-only, dollar-neutral: long `aggregated-scores-best`, short `aggregated-scores-worst`, weighted by a joint Min Variance optimizer (`domain/portfolio_optimizer.py`, optional `portfolio` extra), weekly + monthly rebalance cadences, weekly cron (`.github/workflows/portfolio.yaml`), new dashboard `#portfolio-section`. No stored NAV — weekly returns compounded client-side; no transaction-cost model. [ADR-0012](decisions/0012-longshort-model-portfolio.md), PR [#395](https://github.com/qte77/analyze-stock-kpi/pull/395). Plan [`007`](plans/007-longshort-portfolio-and-multiwindow-sortino.md) (PR B).
- [x] **One qte77 Score everywhere** — `aggregated-scores-best`/`-worst` now rank by `screener_score` ("qte77 Score") instead of the mean of the 7 composites, so a ticker's Score column and rank always agree with its screener_score on every universe; the dashboard's `effectiveScore()`/`meanComposite()` aggregator special case is removed. Also confirms (root-caused via a deterministic interior-NaN-gap test) that #394's `close.dropna()` fix already makes `sortino_ratio` — and therefore `screener_score` — identical for the same ticker regardless of which universe batch it was fetched in.
- [x] **Point-in-time backfilled best/worst 25 + backtested long/short 25/25 — engine, cron + removals** — `orchestrators/longshort_backtest.py` ranks every base universe by a point-in-time-only reduction of the qte77 Score (`score_at`, reusing `screener_score` unchanged) at each weekly rank date since 2023-03-31, and simulates five equal-weight 25/25 cadences (monthly primary, quarterly-after-filings, monthly-with-buffer, weekly, buy-and-hold) with a 10 bp turnover cost, a seeded null benchmark and a live-score fidelity check. Replaces ADR-0012's Min-Variance forward tracker; `domain/portfolio_optimizer.py`, `orchestrators/longshort_portfolio.py` and the `portfolio`/`scipy` extra are removed. New weekly cron (`.github/workflows/portfolio.yaml`, Saturdays 12:00 UTC, full deterministic recompute). [ADR-0013](decisions/0013-point-in-time-backtest.md), PR [#404](https://github.com/qte77/analyze-stock-kpi/pull/404). Plan [`008`](plans/008-pit-longshort-backtest.md) (PR C); the dashboard section shipped separately in PR [#403](https://github.com/qte77/analyze-stock-kpi/pull/403).
- [x] **Backtest: genuine-decision series, freeze, rebalance log + foresight audit** — the headline series A ranks the live qte77 Score on the actual snapshot dates (since 2026-05-31), with the reconstructed backfill kept as a separate series B. Both are frozen append-only, add a `yearly` cadence and a per-cadence rebalance log (when, why, which long/short names changed), and use a 90/120-day US/non-US filing lag. An independent falsify-then-verify audit found no look-ahead, but it did find data defects (NaN scored as 100, negative adjusted closes, rank-date-close fills, a benchmark off-by-one, the series starting in 1962), all fixed before the history was frozen. The dashboard shows A as the headline and B collapsible, and clearing a filter now also clears it from the URL. PRs [#409](https://github.com/qte77/analyze-stock-kpi/pull/409), [#410](https://github.com/qte77/analyze-stock-kpi/pull/410), [#414](https://github.com/qte77/analyze-stock-kpi/pull/414); plan [008](plans/008-pit-longshort-backtest.md).
...
- [x] **Surface "Why these universes?" dashboard tab** — analog of the F&G "Why these charts?" pane; explains best/worst (qte77 Score ranking) vs longshort (conjunctive-gate filter) so the empty-state for unpopulated universes reads as a design outcome, not a bug. Shipped as the `<details id="universe-help">` expander — PR [#235](https://github.com/qte77/analyze-stock-kpi/pull/235).
...
- [x] **Backfill F&G long-term + 5s10s history deeper** — CNN F&G + the 5s10s slope now reach **2011** on the `data` branch (new `scripts/backfill_yield_curve.py` forces a full `period=max` yfinance fetch; F&G via the whit3rabbit mirror), and the demo loader (`loadYearsFromBranch`) now fetches the full year range so the charts paint it. Issue [#287](https://github.com/qte77/analyze-stock-kpi/issues/287), PRs #297 + #300; plan [`backfill-history`](plans/001-backfill-history.md).
- [ ] **Merge F&G long-term + 5s10s into one chart + SP500** (log secondary axis) — collapse the two long-term-context tabs, overlay SP500 price; 5s10s normalized or a third axis. Issue [#288](https://github.com/qte77/analyze-stock-kpi/issues/288), plan [`merge-longterm-charts`](plans/003-merge-longterm-charts.md); depends on #287.
- [x] **Consolidate JS tooling + tests into `ui/`** (Vite build, deploy `dist`) — `package.json` + eslint/prettier/vitest configs + `tests/demo/` moved under `ui/`; Vite emits `ui/dist` and gh-pages deploys it; vendored Chart.js/Fuse.js + `universes.json` live in `ui/public/`. Data stays runtime-fetched. [ADR-0010](decisions/0010-consolidate-js-tooling-into-ui-vite-build.md), issue [#289](https://github.com/qte77/analyze-stock-kpi/issues/289).
- [x] **Restructure data-branch `results/` layout (group by kind)** — regrouped into `series/` + `demo/` + `audit/<universe>/`; coordinated data move (copy → redeploy → delete) kept the live demo 404-free. PR #305 (2026-06-18). Plan [`restructure-results`](plans/005-restructure-results.md); `demo/`→`snapshots/` rename
  deferred, now tracked in [#368](https://github.com/qte77/analyze-stock-kpi/issues/368)
  (originally folded into #289, which shipped without it).
...
- [ ] Deferred EDGAR product use cases — Form 4 insider momentum [#102](https://github.com/qte77/analyze-stock-kpi/issues/102), 8-K material events [#103](https://github.com/qte77/analyze-stock-kpi/issues/103). (XBRL cross-validation [#101](https://github.com/qte77/analyze-stock-kpi/issues/101) promoted out of deferred — see [plan](plans/004-xbrl-crossval.md).)
