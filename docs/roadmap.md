...
- [ ] **Restructure data-branch `results/` layout (group by kind)** — regroup into `series/` + `demo/` + `audit/<universe>/`; coordinated outward-facing data move (copy → redeploy → delete) so the live demo never 404s. Plan [`restructure-results`](plans/restructure-results.md); pre-issue.
