...
- [ ] **Consolidate JS tooling + tests into `ui/`** (Vite build, deploy `dist`) — move `package.json` + configs + `tests/demo/` under `ui/`; follows `../paperverse/ui/`. Issue [#289](https://github.com/qte77/analyze-stock-kpi/issues/289), plan [`consolidate-ui-tooling`](plans/consolidate-ui-tooling.md) (likely own ADR).
