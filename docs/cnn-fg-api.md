...
formal schema; this is the source of truth for what `src/analyze_stock_kpi/data_sources/sentiment.py` may rely
on.
...
`--accent` from CSS at draw time, so dark-mode toggles repaint without a
hover. The monthly chart is lazily constructed on first tab click.
