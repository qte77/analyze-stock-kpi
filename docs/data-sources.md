...
## Redistribution guardrails (verified 2026-05-21; `equity_spy`/SPY row added 2026-06-19; point-in-time backtest row added 2026-09-23)
...
| yfinance closes + statements for the point-in-time backtest | **CAUTION (same as raw payloads above)** | Same Yahoo ToS posture as any other yfinance raw fetch — daily closes and statement line items are "the Services", not public-domain data | **Never commit raw prices or statement line items.** `longshort_backtest.fetch_frames` / `write_closes_cache` cache them only in a gitignored local cache (`results/prices/statements/<ticker>/<fetch-date>.json`, `results/prices/closes/<ticker>.csv`). The `data` branch gets only derived returns, scores, ranks, weights and metrics. See [ADR-0013](decisions/0013-point-in-time-backtest.md) |
