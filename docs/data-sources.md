...
## Redistribution guardrails (verified 2026-05-21; `equity_spy`/SPY row added 2026-06-19; portfolio row added 2026-09-23)
...
| yfinance closes for the long/short model portfolio | **CAUTION (same as raw payloads above)** | Same Yahoo ToS posture as any other yfinance raw fetch — daily closes are "the Services", not public-domain data | **Never commit raw closes.** `longshort_portfolio.fetch_closes` writes them only to a gitignored local cache (`results/prices/closes_<date>.csv`). The `data` branch gets only derived weekly returns (`ret_long`/`ret_short`/`ret_ls`) and target weights. See [ADR-0012](decisions/0012-longshort-model-portfolio.md) |
