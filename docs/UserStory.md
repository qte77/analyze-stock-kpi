...
- **SEC EDGAR enrichment** — `src/analyze_stock_kpi/data_sources/sec/cik_map.py` (CIK ↔ ticker resolver with conditional-GET cache) + `submissions.py` (per-ticker last 10-K / 10-Q / 8-K dates appended to every `FundamentalsSnapshot`).
