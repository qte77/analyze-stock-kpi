...
- **Enhanced KPI long/short conjunctive-gate screener — Phase 2a** (#192). `src/orchestrators/enhanced_kpi_screener_longshort.py` ships 13 of the 16 issue criteria (every gate that reads an existing `FundamentalsSnapshot` field plus the new `analyst_recommendation` field — no new HTTP). Paired output (`enhanced-kpi-screener-longs.txt` + `…-shorts.txt`); a ticker lands in `longs` iff it passes **every** long-side gate, in `shorts` iff it passes **every** inverted short-side gate, otherwise neither. No top-N truncation — counts vary, can be 0 each side. Phase 2b owes criterion 12 (FCF margin, new `Ticker.cashflow` fetch), criterion 15 (tech rating, blocked on #21), and gate-threshold tuning.
