...
- CNN F&G snapshot lands daily in `results/series/cnn_fg/YYYY.json` via cron (v0.4.0 #17).
...
- **Federal-contractors universe** — `usaspending.gov` POST client (#123), `src/analyze_stock_kpi/orchestrators/federal_contractors.py` chains usaspending → EDGAR → yfinance, weekly refresh workflow opens PRs against `main` only when the top-100 ranking diffs. ADR-0006 ([decisions/0006-federal-contractors-universe.md](decisions/0006-federal-contractors-universe.md)).
- **SEC EDGAR enrichment** — `src/analyze_stock_kpi/data_sources/sec/cik_map.py` (CIK ↔ ticker resolver with conditional-GET cache) + `submissions.py` (per-ticker last 10-K / 10-Q / 8-K dates appended to every `FundamentalsSnapshot`) + `xbrl.py` (cross-validates yfinance revenue / net-income / EPS against SEC XBRL `companyconcept` data, attaching `sec_revenue_delta_pct` / `sec_net_income_delta_pct` / `sec_eps_delta_pct`, #101).
...
- **Cross-universe aggregation** — `src/analyze_stock_kpi/orchestrators/aggregated_scores_best_and_worst.py` ranks every ticker across the 7 sector/region universes by its qte77 Score (`screener_score`); emits as a **paired-universes set** (`aggregated-scores-best` + `aggregated-scores-worst`, 25 tickers each). Phase 1 of a long/short-screener pair; ranking agrees with the qte77 Score shown in the dashboard's Score column and detail panel on every universe (previously ranked by the mean of all 7 composites instead, which could disagree with the displayed score). ADR-0005 amendment classifies as Tier-0 pure-aggregation.
- **Enhanced KPI long/short conjunctive-gate screener — Phase 2a + 2b** (#192). `src/analyze_stock_kpi/orchestrators/enhanced_kpi_screener_longshort.py` ships 15 of the 16 issue criteria — Phase 2a delivered 13 gates from existing `FundamentalsSnapshot` fields plus the `analyst_recommendation` bucket; Phase 2b added criterion 12 (FCF margin via `Ticker.cashflow` / `Ticker.income_stmt`, attached as the `fcf_margin` enrichment). Paired output (`enhanced-kpi-screener-longs.txt` + `…-shorts.txt`); a ticker lands in `longs` iff it passes **every** long-side gate, in `shorts` iff it passes **every** inverted short-side gate, otherwise neither. No top-N truncation — counts vary, can be 0 each side. Criterion 15 (tech rating) stays deferred behind the #21 TradingView decision.
- **Multi-window Sortino** (plan [007](plans/007-longshort-portfolio-and-multiwindow-sortino.md)). Per-ticker `sortino_ratio` (1y, unchanged composite input) gains informational `sortino_3y/5y/10y/20y/30y` windows plus a CLI-only custom frame (`--sortino-from`/`--sortino-to`).
- **Point-in-time backfilled best/worst 25 + backtested long/short 25/25** (plan [008](plans/008-pit-longshort-backtest.md), superseding plan 007's Min-Variance model portfolio). Two series, never joined into one line:
  - **Genuine decisions (the headline).** The live qte77 Score's best/worst 25 on each actual snapshot date since 2026-05-31, traded at each ticker's own next close.
  - **Reconstructed backfill (an approximation).** Every base universe is re-ranked by a point-in-time-only reduction of the qte77 Score (no forward-looking valuation/beta inputs) at each weekly rank date since 2023-05-05. This is the aggregated best/worst 25 **as they would have been** on that date, not today's ranking applied retroactively.

  A hypothetical, equal-weight 1/25 long-best/short-worst book is backtested across six cadences (monthly primary, quarterly-after-filings, monthly-with-buffer, weekly, yearly, buy-and-hold), gross and net of a 10 bp turnover cost. It comes with a seeded null benchmark, a live-score fidelity check, and a rebalance log showing when, why, and which long/short names changed. History is frozen append-only, and an independent audit (2026-09-24) found no look-ahead. Survivorship bias and other caveats are disclosed, not hidden; explicitly framed as hypothetical and backward-looking, not investment advice. [ADR-0013](decisions/0013-point-in-time-backtest.md).
