...
- **Multi-window Sortino + hypothetical long/short model portfolio** (plan [007](plans/007-longshort-portfolio-and-multiwindow-sortino.md)). Per-ticker `sortino_ratio` (1y, unchanged composite input) gains informational `sortino_3y/5y/10y/20y/30y` windows plus a CLI-only custom frame (`--sortino-from`/`--sortino-to`). Separately, a forward-only, dollar-neutral **model portfolio** — long `aggregated-scores-best`, short `aggregated-scores-worst`, weighted by a joint Min Variance optimizer (optional `portfolio` extra, `scipy`) — is tracked weekly and monthly via a new cron and dashboard section. No backtest, no stored NAV, no transaction-cost model; explicitly framed as hypothetical, not investment advice. [ADR-0012](decisions/0012-longshort-model-portfolio.md).
