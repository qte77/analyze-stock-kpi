...
- **Demo: Long-Term Context section · monthly Fear & Greed (median + average).** New `<section id="long-term-section">` between `#fear-greed-section` and `#universe-section` showing per-month median (solid, `--text`) and average (dashed, `--accent`) reductions of the existing `loadFearGreedYears()` data — no new fetch. New pure module `docs/demo/lib/monthly.js::aggregateMonthlyFG` buckets by UTC `YYYY-MM` so boundaries stay stable across viewer timezones; empty F&G fetch renders a centered "no F&G history yet" hint instead of a blank canvas. 8 new Vitest cases (TDD red→green). Documented under "Backfillable on any cron run" in `docs/cnn-fg-api.md`.
