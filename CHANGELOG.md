...
- **`docs/data-sources.md` — Redistribution guardrails section (verified 2026-05-21).** Independent ToS / license audit per source for committing derived outputs to the public `data` branch. Verdicts: usaspending.gov **CLEAR** (CC0 + DATA Act), SEC EDGAR **CLEAR** (17 USC § 105 federal-works public-domain), yfinance raw payloads **CAUTION** (Yahoo ToS §2.4(i)/§2.8 prohibits redistribution; derived ticker list + resolution boolean **CLEAR**), `fedspendingtransparency/usaspending-api` API contract files **CLEAR** (CC0; verbatim fixture copy OK), CNN F&G numeric values **CAUTION** (ToS-breach risk on access, not copyright). Standing rule that lands with this: `AuditRow` in the upcoming `src/federal_contractors.py` MUST NOT carry any `fast_info` / `info` fields — only the ticker symbol + boolean.
