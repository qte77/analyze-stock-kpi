...
- **Demo v2 simple/detailed toggle dashboard (#134).** `docs/demo/index.html` gains a `<button id="view-toggle">` next to the freshness badge; body `view-simple` / `view-detailed` class flip drives a CSS `.detail-only` rule so 8 of 13 KPI columns + the detail-panel aside + the CSV export button hide in simple mode. Simple-mode row-click opens Yahoo Finance directly; detailed-mode opens the full detail panel with a new composite-score radar (B1) and external-link nav (Yahoo / SEC EDGAR / Wikipedia, A4). Universe header gains a sector breakdown donut (B3) in both modes. Conditional cell coloring (A1) across 10 numeric KPIs via threshold map. Full URL state persistence (A2) — `?view`, `?universe`, `?sort`, `?sortDir`, `?filter`, `?date` all round-trip via `history.replaceState`; view-mode also persists to `localStorage`. Empty / loading states (A5), `/` keyboard shortcut to focus filter (D1), `prefers-color-scheme: dark` block remapping CSS variables (D2), `<768px` viewport auto-engages simple mode (D3). New `package.json` + `vitest.config.mjs` + `make test_js` wires Vitest as the JS unit-test framework (closes #131); pure-JS units extracted into `docs/demo/lib/{audit,coloring,csv,sector,state}.js` with non-trivial unit coverage. ESLint + Prettier deferred per #135. Strict-lint tsconfig flip deferred to a follow-on commit.
